...
class ArxivUrlTest(unittest.TestCase):
    ...
    def test_get_date_range_param(self):
        ...
        self.assertEqual(arxiv_url.get_date_range_param(start, end), 'date-from_date=2018-06-08&date-to_date=2018-07-08')

    def test_get_size_param(self):
        size = 50
        self.assertEqual(arxiv_url.get_size_param(size), 'size=50')
        size += 1
        self.assertEqual(arxiv_url.get_size_param(size), 'size=51')

    def test_get_order_param(self):
        order = "announced_date_first"
        self.assertEqual(arxiv_url.get_order_param(order=order), 'order=-announced_date_first')
